			...
			?><tr id="lead_row_<?php echo $lead["id"] ?>" class='<?php echo trim($rowclass.$evenodd); echo $lead["is_starred"] ? " featured" : "" ?>'<?php echo $rowstyle . $valignattr; ?>><?php
				...
				foreach($field_ids as $field_id) {
					...
					switch($input_type){
						...
						case "post_title":
							if( apply_filters( 'kws_gf_directory_post_title_link', false, $form ) && !empty( $lead['post_id'] ) ) {
								$value = '<a href="'. esc_url( get_permalink( $lead['post_id'] ) ) .'" title="'. esc_attr( $value ).'">'. esc_html( $value ) .'</a>';
							} else {
